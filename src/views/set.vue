<template>
  <main id="register" class="view" role="main">
    <div class="register">
      <md-field>
        <md-field-item style="background-color:#6799AF;" name="item0" title="设置密码" solid>
          <md-icon name="arrowl" slot="left" @click.native="back" class="md-icon"></md-icon>
        </md-field-item>
      </md-field>
      <md-field class="input">
        <md-input-item v-model="form.password" type="password" ref="password" placeholder="请输入密码" clearable>
        </md-input-item>
      </md-field>
...
export default {
  ...
  methods: {
    ...
    next () {
      const sign = this.validatepass(this.form.password)
      if (!sign) {
        this.$message({
          type: 'warning',
          message: '格式不正确'
        })
      } else if (this.insurepass !== this.form.password) {
        this.$message({
          type: 'warning',
          message: '两次输入密码不一致'
        })
      } else {
        axios.post('http://101.132.44.253:8762/userRegister', this.form, {
          headers: {
